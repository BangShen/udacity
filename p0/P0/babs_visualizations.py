...
def filter_data(data, condition):#这个函数是要求读者写出condition然后会对这个data进行筛选并且把筛选之后的数据方法哦data里面
    """
    ...
    field, op, value = condition.split(" ", 2)#这个2的意思是只分离前两个有空格的这个东西,也就是分了两个空格后就不会再分了，后面的就会在一
    #个‘ ’后面,注意里面用了一个比较高级的赋值方法，通过dilimeter从而将condition分成了3个部分，并且将其赋值给3个变量
    ...
    data = data[matches].reset_index(drop = True)#不是太懂
    return data
...
def usage_stats(data, filters = [], verbose = True):
    ...
    data['duration']=timedelta(minutes=data['duration'])
